...
x_train1 = iris.x[:75,0:2]
x_train2 = iris.x[:75,2:4]
y_train = to_categorical(iris.y[:75],3) # convert our responses to categories
        
x_test1 = iris.x[74:,0:2]
x_test2 = iris.x[74:,2:4]
y_test = to_categorical(iris.y[74:],3)


print("\nBuilding model...")

iris_input_left = Input(shape=(2,), name='iris_input_left')
iris_input_right = Input(shape=(2,), name='iris_input_right')

#left branch
xl = Dense(200, name='ldense_layer1')(iris_input_left)
xl = Dense(100, name='ldense_layer2')(xl)

#right branch
xr = Dense(200, name='rdense_layer1')(iris_input_right)
xr = Dense(100, name='rdense_layer2')(xr)

merge_outputs = Concatenate(name="merge_layer")([xl, xr])
iris_class_outputs = Dense(3,activation='softmax')(merge_outputs)   #    <  0  ,  0,  0  >
           
model = Model(inputs=[iris_input_left, iris_input_right], outputs=iris_class_outputs, name="iris_model")
...
print("\nTraining now...")
model_info = model.fit([x_train1, x_train2], y_train, 
                       epochs=num_epochs,
...
print("\nMaking predictions...")
loss, acc = model.evaluate([x_test1, x_test2],y_test,verbose=0)
